...
echo "=== Calico QoS Lab Cleanup ==="

echo "=== Destroying ContainerLab topology ==="
sudo containerlab destroy -t calico-qos.clab.yaml || { echo "Warning: Failed to destroy topology (may not exist)"; }

echo "=== Deleting Kind cluster ==="
kind delete cluster --name=calico-qos || { echo "Warning: Failed to delete Kind cluster (may not exist)"; }

echo "=== Verifying cleanup ==="
echo "Checking for remaining containers..."
docker ps | grep calico-qos || echo "No calico-qos containers found."

echo "Checking for remaining networks..."
docker network ls | grep clab || echo "No clab networks found."

echo "=== Cleaning up local files ==="
rm -f calico-qos.kubeconfig
