...
sudo kind get kubeconfig --name=headless-services > headless-services.kubeconfig
sudo chmod 644 headless-services.kubeconfig

# Use the specific kubeconfig file for all kubectl commands
export KUBECONFIG=$(pwd)/headless-services.kubeconfig
...
echo "export KUBECONFIG=$(pwd)/headless-services.kubeconfig"
export KUBECONFIG=$(pwd)/headless-services.kubeconfig
