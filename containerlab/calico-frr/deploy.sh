#!/bin/bash
# filepath: /Users/aadhilamajeed/k824/container-labs/containerlab/calico-overlay/deploy.sh

set -e  # Exit immediately if a command exits with a non-zero status

echo "=== Destroying existing ContainerLab topology ==="
sudo containerlab destroy -t containerlab/topology.clab.yaml || { echo "Failed to destroy existing topology"; exit 1; }

echo "=== Deploying ContainerLab topology with FRR ==="
sudo containerlab deploy -t containerlab/topology.clab.yaml || { echo "Failed to deploy topology"; exit 1; }
...
echo "=== Configuring FRR BGP ==="
echo "Checking FRR status..."
...
echo ""
echo "To check FRR BGP status, run:"
echo "sudo docker exec clab-calico-overlay-frr01 vtysh -c 'show bgp summary'"
echo "sudo docker exec clab-calico-overlay-frr01 vtysh -c 'show ip route bgp'"