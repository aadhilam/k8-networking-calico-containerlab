...
sudo containerlab destroy -t k8-services.clab.yaml || { echo "Failed to destroy existing topology"; exit 1; }

echo "=== Deploying ContainerLab topology ==="
sudo containerlab deploy -t k8-services.clab.yaml || { echo "Failed to deploy topology"; exit 1; }
...
sudo kind get kubeconfig --name=k8-services > k8-services.kubeconfig
sudo chmod 644 k8-services.kubeconfig

# Use the specific kubeconfig file for all kubectl commands
export KUBECONFIG=$(pwd)/k8-services.kubeconfig
...
echo "export KUBECONFIG=$(pwd)/k8-services.kubeconfig"
export KUBECONFIG=$(pwd)/k8-services.kubeconfig