...
sudo containerlab destroy -t k8s-dns.clab.yaml || { echo "Failed to destroy existing topology"; exit 1; }

echo "=== Deploying ContainerLab topology ==="
sudo containerlab deploy -t k8s-dns.clab.yaml || { echo "Failed to deploy topology"; exit 1; }
...
sudo kind get kubeconfig --name=k8s-dns > k8s-dns.kubeconfig
sudo chmod 644 k8s-dns.kubeconfig

# Use the specific kubeconfig file for all kubectl commands
export KUBECONFIG=$(pwd)/k8s-dns.kubeconfig
...
echo "export KUBECONFIG=$(pwd)/k8s-dns.kubeconfig"
export KUBECONFIG=$(pwd)/k8s-dns.kubeconfig